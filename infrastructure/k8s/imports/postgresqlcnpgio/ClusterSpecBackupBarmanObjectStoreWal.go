...
type ClusterSpecBackupBarmanObjectStoreWal struct {
	...
	Compression ClusterSpecBackupBarmanObjectStoreWalCompression `field:"optional" json:"compression" yaml:"compression"`
	// Whenever to force the encryption of files (if the bucket is not already configured for that).
	//
	// Allowed options are empty string (use the bucket policy, default),
	// `AES256` and `aws:kms`.
	Encryption ClusterSpecBackupBarmanObjectStoreWalEncryption `field:"optional" json:"encryption" yaml:"encryption"`
