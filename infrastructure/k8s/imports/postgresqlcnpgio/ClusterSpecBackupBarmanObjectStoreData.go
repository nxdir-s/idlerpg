...
type ClusterSpecBackupBarmanObjectStoreData struct {
	...
	Compression ClusterSpecBackupBarmanObjectStoreDataCompression `field:"optional" json:"compression" yaml:"compression"`
	// Whenever to force the encryption of files (if the bucket is not already configured for that).
	//
	// Allowed options are empty string (use the bucket policy, default),
	// `AES256` and `aws:kms`.
	Encryption ClusterSpecBackupBarmanObjectStoreDataEncryption `field:"optional" json:"encryption" yaml:"encryption"`
